...
class HMM(object):
    def __init__(self, N, M):
        """Args:
            N: 状态数，这里对应存在的标注的种类 tag2id
            M: 观测数，这里对应有多少不同的字 word2id
            隐马尔可夫模型（HMM）可以用五个元素来描述，
            包括2个状态集合和3个概率矩阵：
            1. 隐含状态 S、2. 可观测状态 O、
            3. 初始状态概率矩阵 π、4. 隐含状态转移概率矩阵 A、5. 观测状态转移概率矩阵 B 。
        """
        self.N = N
        self.M = M
    ...
    def decoding(self, word_list, word2id, tag2id):
        """
        使用维特比算法对给定观测序列求状态序列（预测状态）， 这里就是对字组成的序列,求其对应的标注。
