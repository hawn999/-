...
class BILSTM_Model(object):
    ...
    def train(self, word_lists, tag_lists,
              ...
              word2id, tag2id):
        x_epoch=[i+1 for i in range(self.epoches)]
        y_loss=[]
        # 对数据集按照长度进行排序
        ...
        for e in range(1, self.epoches+1):
            ...
            y_loss.append(val_loss)

        plt.plot(x_epoch,y_loss)
        print("start drawing")
        plt.xlabel("epoch")
        plt.ylabel("val_loss")
        plt.title("learning rate:{}  epoches:{}".format(self.lr,self.epoches))
        plt.show()
        if self.crf:
            plt.savefig("bilstm_crf_learning rate:{}_epoches:{}.png".format(self.lr,self.epoches))
        else:
            plt.savefig("bilstm_learning rate:{}_epoches:{}.png".format(self.lr, self.epoches))
    ...
    def test2(self, word_lists, word2id, tag2id):
        """返回最佳模型在测试集上的预测结果"""
        # 准备数据
        # word_lists, tag_lists, indices = sort_by_lengths(word_lists, tag_lists)
        tensorized_sents, lengths = tensorized(word_lists, word2id)
